...
export async function getPwaSpecs(): Promise<SpecSummary[]> {
  ...
  const pwaSpecs = allSpecs.filter(spec => {
    ...
    const matchesShortname = PWA_SHORTNAMES.some(name =>
      spec.shortname.includes(name) ||
      spec.series?.shortname?.includes(name)
    );
    ...
    const matchesKeyword = PWA_KEYWORDS.some(keyword =>
  ...
  pwaSpecs.sort((a, b) => {
    const aExact = PWA_SHORTNAMES.includes(a.shortname) || PWA_SHORTNAMES.includes(a.series?.shortname || '');
    const bExact = PWA_SHORTNAMES.includes(b.shortname) || PWA_SHORTNAMES.includes(b.series?.shortname || '');
